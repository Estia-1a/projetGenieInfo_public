...
data.map( e => {
    const l = students[ cleanPath(e.leftFilePath) ] = students[ cleanPath(e.leftFilePath) ]  ?? { triche : false, cluster : null, link : 0 , similarity : 0  , all:{}}
    const r = students[ cleanPath(e.rightFilePath) ] = students[ cleanPath(e.rightFilePath) ]  ?? { triche : false, cluster : null, link : 0 , similarity : 0 , all:{} }
...
Object.entries( students ).map( async ([path, info ]) => {
    if( info.triche ) {
        ...
        Object.entries( info.all ).map( ([path, sim ] ) => copyFile( "result/" + path + ".js", dest + "/" +  "_" + Math.floor((1 - sim)*10)  + path + ".js" ) );
    }
})
